...
//WebUI.callTestCase(findTestCase('3-Main_Menu/2-sorting_name_(Z-to-A)'), [:], FailureHandling.STOP_ON_FAILURE)

// Definisikan XPath yang ingin digunakan
String xpathExpression = '//button[contains(@name,"add-to-cart-")]'
...
// Buat list untuk menyimpan informasi button di setiap produk
...
if (productElements.size() == 6) {
    ...
    for (int i = 0; i < maxProductsToCheck; i++) {
        ...
        if (element.isDisplayed()) {
            KeywordUtil.logInfo('Button terlihat di halaman website.')
        } else {
            KeywordUtil.logInfo('Button TIDAK terlihat di halaman website.')
        }
    }
    
    // Cetak hasil pengecekan button yang diambil dari halaman untuk verifikasi
    KeywordUtil.logInfo('Button yang terlihat:\n' + productNames.join('\n' // Jika kurang dari 6 elemen, cetak pesan dan hentikan eksekusi
            ))
} else {
