...
//WebUI.callTestCase(findTestCase('3-Main_Menu/6-verify the product have price'), [:], FailureHandling.STOP_ON_FAILURE)

// Definisikan XPath yang ingin digunakan
String xpathExpression = '//div[@class="inventory_item_img"]//img[contains(@src,"" ) and contains(@alt,"")]'
...
if (productElements.size() == 6) {
    ...
    for (int i = 0; i < maxProductsToCheck; i++) {
        ...
        productImages.add(productName + '\n')

        // Verifikasi bahwa elemen terlihat (dari hasil pengecekan sebelumnya)
        if (element.isDisplayed()) {
            KeywordUtil.logInfo('Images Produk terlihat di halaman website.')
        } else {
            KeywordUtil.logInfo('Images Produk TIDAK terlihat di halaman website.')
        }
