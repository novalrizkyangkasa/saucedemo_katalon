...
//WebUI.callTestCase(findTestCase('3-Main_Menu/5-verify the product have description'), [:], FailureHandling.STOP_ON_FAILURE)

// Definisikan XPath yang ingin digunakan
String xpathExpression = '//div[@id="inventory_container"]//div[@data-test="inventory-item-price"]'
...
// Buat list untuk menyimpan harga produk
List<WebElement> productprice = new ArrayList()
...
if (productElements.size() == 6) {
    ...
    for (int i = 0; i < maxProductsToCheck; i++) {
        ...
        productprice.add(productName + '\n')

        // Verifikasi bahwa elemen terlihat (dari hasil pengecekan sebelumnya)
        if (element.isDisplayed()) {
            KeywordUtil.logInfo(('Harga ' + productName) + ' terlihat di halaman website.')
        } else {
            KeywordUtil.logInfo(('Harga ' + productName) + ' TIDAK terlihat di halaman website.')
        }
    }
    
    // Cetak list harga produk yang diambil dari halaman untuk verifikasi
    KeywordUtil.logInfo('Harga produk yang terlihat:\n' + productprice.join('\n') // Jika kurang dari 6 elemen, cetak pesan dan hentikan eksekusi
        )
} else {
